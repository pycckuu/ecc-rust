...
/// Represents a curve in short Weierstrass form: y^2 = x^3 + ax + b
...
impl EllipticCurve for WeierstrassCurve {
  ...
  fn is_on_curve(&self, a: &Point) -> bool {
      match a {
          Point::Coordinates(x, y) => {
...
impl WeierstrassCurve {
  pub fn new(a: BigUint, b: BigUint, p: BigUint, n: BigUint, g: Point) -> Self {
      WeierstrassCurve { a, b, p, n, g }
  }
...
mod tests {
  ...
  fn create_test_curve() -> WeierstrassCurve {
      WeierstrassCurve {
          ...
          n: BigUint::from(18u32),
          g: Point::Coordinates(BigUint::from(5u32), BigUint::from(1u32)),
      }
  }
  ...
  mod curve_properties {
    ...
    //     #[test]
    //     fn test_is_on_curve() {
    //         let curve = create_test_montgomery_curve();
    //         let p = curve.base_point().clone();
    //         assert!(curve.is_on_curve(&p));

    //         let not_on_curve = Point::Coordinates(BigUint::from(3u32), BigUint::from(4u32));
    //         assert!(!curve.is_on_curve(&not_on_curve));
    //     }

    //     // Add more tests for Montgomery curve operations
    // }
    ...
    //     #[test]
    //     fn test_is_on_curve() {
    //         let curve = create_test_edwards_curve();
    //         let p = curve.base_point().clone();
    //         assert!(curve.is_on_curve(&p));
    ...
    //     // Add more tests for Edwards curve operations
    // }
// }
